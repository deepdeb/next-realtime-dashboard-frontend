...
export default function Revenue() {
    ...
    return (
        <div className="flex flex-col items-center space-y-5 w-full p-10">
            <div className="grid grid-cols-2 gap-2 w-full">
                <div className="rounded-xl bg-zinc-100 p-4 text-sky-600 w-full flex items-center justify-center">
                    <div>
                ...
                <div className="grid gap-2">
                    <div className="rounded-xl bg-zinc-100 p-4 text-green-600 w-full">
                        <p className="text-center">Total Profits</p>
                        <p className="text-center">{totalProfits}</p>
                    </div>
                    <div className="rounded-xl bg-zinc-100 p-4 text-red-600 w-full">
            ...
            <div className="grid grid-cols-2 gap-2 w-full">
                <div className="grid gap-2">
                    <div
                        className={`rounded-xl bg-zinc-100 p-4 ${percentageChange?.changeProfit! > 0
                            ? "text-green-600"
                            : "text-red-600"
                            } w-full`}
                    >
                        ...
                        <p className="text-center">
                            {percentageChange?.changeProfit! > 0 ? <span>&uarr;</span> : <span>&darr;</span>}
                            {percentageChange?.changeProfit.toFixed(2)}
                        </p>
                    </div>
                    <div
                        className={`rounded-xl bg-zinc-100 p-4 ${percentageChange?.changeLoss! < 0
                            ? "text-green-600"
                            : "text-red-600"
                            } w-full`}
                    >
